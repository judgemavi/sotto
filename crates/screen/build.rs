...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!("cargo:rerun-if-changed=bridge-macos/Sources/SottoScreenBridge/FrameExtractor.swift");
    ...
    let status = Command::new("swift")
        ...
        .status()?;
    if !status.success() {
        return Err(format!("Swift screen bridge build failed with {status}").into());
    }
    ...
    println!("cargo:rustc-link-lib=static=SottoScreenBridge");
    for framework in [
        "AppKit",
        "AVFoundation",
        "CoreGraphics",
